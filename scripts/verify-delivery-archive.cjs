...
const filename = path.join(root, 'release', 'SusuAIOverclock-1.5.7-portable-electron44.4.3-isolated.zip');
...
if (binaries.length !== 1 || path.posix.basename(binaries[0].name) !== 'SusuAIOverclock-1.5.7-portable.exe') throw new Error('Unexpected executable members');
...
const reportPath = path.join(root, 'release', '1.5.7-DELIVERY-INTEGRITY.json');
